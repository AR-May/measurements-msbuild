...
.DESCRIPTION
    ...
    performance pipeline (definition 25429) and copies JSON files into the data/ folder.
...
foreach ($build in $builds) {
    $buildId = $build.id
    # Extract date from buildNumber (format: YYYYMMDD.N.s_...)
    $buildDate = $build.buildNumber -replace '^(\d{8})\..*', '$1'
    ...
    foreach ($artifactName in $ArtifactNames) {
        ...
        $destDir = Join-Path $DataDir (Join-Path $buildDate $machine)

        if (Test-Path $destDir) {
            Write-Host "  [skip] $buildDate/$machine already exists" -ForegroundColor DarkGray
            continue
        }

        Write-Host "  Downloading $artifactName from build $buildId ($buildDate, $($build.result)) ..." -ForegroundColor Yellow
        ...
        Write-Host "  [done] $($jsonFiles.Count) JSON files -> data/$buildDate/$machine" -ForegroundColor Green
        $downloaded++
